...
public class GameStart extends Activity implements SensorEventListener {
    static final Object baton = new Object();
    ImageButton killButton;
    ImageButton repelButton;
    ...
    protected void onResume() {
        ...
        positionUpdater = new MoveRunnable(currentMap);
    ...
    private void loadGameDataFromSharedPreferences(SharedPreferences preferences) {
        ...
        // stats
        int moneyStart = 15 - DiffSetting.difficulty;
        ...
        String[] allGhostInfo = ghostInfo.substring(0, ghostInfo.length())
                .split(";");
        ArrayList<Ghost> ghost1List = new ArrayList<>();
        ArrayList<Ghost> ghost2List = new ArrayList<>();

        try {
            for (String anAllGhostInfo : allGhostInfo) {
                String[] oneGhostInfo = anAllGhostInfo.split(",");
    ...
    private void addButtonsToLayout() {
        ...
        repelButton.setImageDrawable(ContextCompat.getDrawable(this, R.drawable.repellent_off));
    ...
    class Repel implements OnClickListener {
        ...
        public void onClick(View arg0) {
            repeller = 0;
            if (!spray && GameStart.money > 0) {
                for (Ghost ghost : MapView.ghost1List) {
                    ghost.setV(-2);
                }
                spray = true;
                repelButton.setImageDrawable(ContextCompat.getDrawable(GameStart.this, R.drawable.repellent_on));
            } else if (spray || GameStart.money == 0) {
                spray = false;
                repeller = 0;
                for (Ghost ghost : MapView.ghost1List) {
                    ghost.setV(2);
                }
                repelButton.setImageDrawable(ContextCompat.getDrawable(GameStart.this, R.drawable.repellent_off));
            }
