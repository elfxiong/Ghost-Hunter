...
public class Coin extends View {
	...
	public Coin(float x, float y, Context context, AttributeSet attrs) {
		super(context, attrs);
		init(x, y);
	}
	...
	public static void collectCoin(Human human, ArrayList<Coin> coinList) {
		for (int i = 0; i < coinList.size(); i++){
		if (human.getX() < coinList.get(i).getX() + 16 && human.getX() > coinList.get(i).getX() - 16
				|| human.getY() < coinList.get(i).getY() + 16 && human.getY() > coinList.get(i).getY() - 16) {
			coinList.remove(i);
			GameStart.money += 1;
		}
	}
